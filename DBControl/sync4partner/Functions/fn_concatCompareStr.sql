...
BEGIN
 ...
 select @c = @c + '((ISNULL([' + cName + '])!=ISNULL(['+cName+' (1)]))||((!ISNULL([' + cName + '])&&!ISNULL(['+cName+' (1)]))&&(['+cName+']!=['+cName+' (1)]))) ||'
  ...
  where is_primarykey = 0 and is_nullable = 1 and typeName <> 'timestamp' and tname = @tableName and is_computed=0

  select @c = @c +  '(['+cName+']!=['+cName+' (1)]) ||'
   from [sync4partner].[ColumnList]
   where is_primarykey = 0 and is_nullable = 0 and typeName <> 'timestamp' and tname = @tableName and is_computed=0
